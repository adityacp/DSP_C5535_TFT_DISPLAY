...
*/
Adafruit_SPITFT::Adafruit_SPITFT(Uint16 w, Uint16 h,
  Int8 cs, Int8 dc, Int8 mosi, Int8 sck, Int8 rst, Int8 miso) :
...
//Adafruit_SPITFT::Adafruit_SPITFT(Uint16 w, Uint16 h, Int8 cs,
//  Int8 dc, Int8 rst) : Adafruit_GFX(w, h),
...
//*/
//Adafruit_SPITFT::Adafruit_SPITFT(Uint16 w, Uint16 h, SPIClass *spiClass,
//  Int8 cs, Int8 dc, Int8 rst) : Adafruit_GFX(w, h),
//  connection(TFT_HARD_SPI), _rst(rst), _cs(cs), _dc(dc) {
//    hwspi._spi = spiClass;
//#if defined(USE_FAST_PINIO)
// #if defined(HAS_PORT_SET_CLR)
...
*/
Adafruit_SPITFT::Adafruit_SPITFT(Uint16 w, Uint16 h, tftBusWidth busWidth,
  Int8 d0, Int8 wr, Int8 dc, Int8 cs, Int8 rst, Int8 rd) :
  Adafruit_GFX(w, h), connection(TFT_PARALLEL), _rst(rst), _cs(cs), _dc(dc) {
    ...
    // These are all Uint8* pointers -- elsewhere they're recast
    ...
    Uint8       dBit    = g_APinDescription[d0].ulPin; // d0 bit # in PORT
    PortGroup    *p       = (&(PORT->Group[g_APinDescription[d0].ulPort]));
    Uint8       offset  = dBit / 8; // d[7:0] byte # within PORT
    if(tft8.wide) offset &= ~1; // d[15:8] byte # within PORT
    // These are all Uint8* pointers -- elsewhere they're recast
    // as necessary if a 'wide' 16-bit interface is in use.
    tft8.writePort    = (volatile Uint8 *)&(p->OUT.reg)    + offset;
    tft8.readPort     = (volatile Uint8 *)&(p->IN.reg)     + offset;
    tft8.dirSet       = (volatile Uint8 *)&(p->DIRSET.reg) + offset;
    tft8.dirClr       = (volatile Uint8 *)&(p->DIRCLR.reg) + offset;
  #endif // end !CORE_TEENSY
 #else  // !HAS_PORT_SET_CLR
...
*/
void Adafruit_SPITFT::initSPI(Uint32 freq, Uint8 spiMode) {

    if(!freq) freq = DEFAULT_SPI_FREQ; // If no freq specified, use default

    // Init basic control pins common to all connection types
    if(_cs >= 0) {
        USBSTK5515_GPIO_setDirection(_cs, OUTPUT);
        USBSTK5515_GPIO_setOutput(_cs, HIGH); // Deselect
    }
    USBSTK5515_GPIO_setDirection(_dc, OUTPUT);
    USBSTK5515_GPIO_setOutput(_dc, HIGH); // Data mode
...
//        }
        ...
        USBSTK5515_GPIO_setDirection(swspi._mosi, OUTPUT);
        USBSTK5515_GPIO_setOutput(swspi._mosi, LOW);
        USBSTK5515_GPIO_setDirection(swspi._sck, OUTPUT);
        USBSTK5515_GPIO_setOutput(swspi._sck, LOW);
        if(swspi._miso >= 0) {
            USBSTK5515_GPIO_setDirection(swspi._miso, INPUT);
        }
        ...
        for(Uint8 i=0; i<NUM_DIGITAL_PINS; i++) {
        ...
        if(!tft8.wide) {
            ...
            *(volatile Uint16 *)tft8.dirSet    = 0xFFFF;
            *(volatile Uint16 *)tft8.writePort = 0x0000;
        }
 #else  // !CORE_TEENSY
        Uint8 portNum = g_APinDescription[tft8._d0].ulPort, // d0 PORT #
                dBit    = g_APinDescription[tft8._d0].ulPin,  // d0 bit in PORT
                lastBit = dBit + (tft8.wide ? 15 : 7);
        for(Uint8 i=0; i<PINS_COUNT; i++) {
            if((g_APinDescription[i].ulPort == portNum ) &&
               (g_APinDescription[i].ulPin  >= dBit    ) &&
               (g_APinDescription[i].ulPin  <= (Uint32)lastBit)) {
                pinMode(i, OUTPUT);
                digitalWrite(i, LOW);
            }
        }
 #endif // end !CORE_TEENSY
#endif
        USBSTK5515_GPIO_setDirection(tft8._wr, OUTPUT);
        USBSTK5515_GPIO_setOutput(tft8._wr, HIGH);
        if(tft8._rd >= 0) {
            USBSTK5515_GPIO_setDirection(tft8._rd, OUTPUT);
            USBSTK5515_GPIO_setOutput(tft8._rd, HIGH);
        }
    }

    if(_rst >= 0) {
        // Toggle _rst low to reset
        USBSTK5515_GPIO_setDirection(_rst, OUTPUT);
        USBSTK5515_GPIO_setOutput(_rst, HIGH);
        USBSTK5515_wait(100);
        USBSTK5515_GPIO_setOutput(_rst, LOW);
        USBSTK5515_wait(100);
        USBSTK5515_GPIO_setOutput(_rst, HIGH);
        USBSTK5515_wait(200);
    }
    ...
    if(((connection == TFT_HARD_SPI) || (connection == TFT_PARALLEL)) &&
       (dma.allocate() == DMA_STATUS_OK)) { // Allocate channel
        ...
        if(dptr = dma.addDescriptor(NULL, NULL, 42, DMA_BEAT_SIZE_BYTE,
            ...
            if((pixelBuf[0] =
              (Uint16 *)malloc(maxFillLen * sizeof(Uint16)))) {
...
*/
void Adafruit_SPITFT::writePixel(Int16 x, Int16 y, Uint16 color) {
...
/*!
    ...
    @param  bigEndian  If using DMA, and if set true, bitmap in memory is in
                       ...
                       example, a bitmap in a Uint16 array having the byte
...
*/
void Adafruit_SPITFT::writePixels(Uint16 *colors, Uint32 len,
  Bool block, Bool bigEndian) {
    ...
    if((connection == TFT_HARD_SPI) || (connection == TFT_PARALLEL)) {
        ...
        Uint8 pixelBufIdx = 0;              // Active pixel buffer number
 #if defined(__SAMD51__)
        ...
        if(!bigEndian) { // Normal little-endian situation...
            while(len) {
                ...
                descriptor[pixelBufIdx].SRCADDR.reg       =
                  (Uint32)pixelBuf[pixelBufIdx] + count * 2;
            ...
            for(d=0; d<numDescriptors; d++) {
                ...
                descriptor[d].SRCADDR.reg       = (Uint32)colors + count * 2;
                descriptor[d].BTCTRL.bit.SRCINC = 1;
                descriptor[d].BTCNT.reg         = count * 2;
                descriptor[d].DESCADDR.reg      = (Uint32)&descriptor[d+1];
                len    -= count;
                colors += count;
            }
...
*/
void Adafruit_SPITFT::writeColor(Uint16 color, Uint32 len) {
    ...
    if(connection == TFT_HARD_SPI) {
        ...
        static Uint32 temp[TMPBUF_LONGWORDS];
        Uint32        c32    = color * 0x00010001;
        Uint16        bufLen = (len < TMPBUF_PIXELS) ? len : TMPBUF_PIXELS,
                        xferLen, fillLen;
        // Fill temp buffer 32 bits at a time
        fillLen = (bufLen + 1) / 2; // Round up to next 32-bit boundary
        for(Uint32 t=0; t<fillLen; t++) {
            temp[t] = c32;
        }
        // Issue pixels in blocks from temp buffer
        while(len) {                                 // While pixels remain
            xferLen = (bufLen < len) ? bufLen : len; // How many this pass?
            writePixels((Uint16 *)temp, xferLen);
            len -= xferLen;
        }
        return;
    }
    ...
    if(((connection == TFT_HARD_SPI) || (connection == TFT_PARALLEL)) &&
       (len >= 16)) { // Don't bother with DMA on short pixel runs
        ...
        if(hi == lo) { // If high & low bytes are same...
            ...
            for(d=0; d<numDescriptors; d++) {
                int count = (len < 32767) ? len : 32767;
                descriptor[d].SRCADDR.reg       = (Uint32)&onePixelBuf;
            ...
            Uint32 *pixelPtr  = (Uint32 *)pixelBuf[0],
            ...
            for(d=0; d<numDescriptors; d++) {
                int pixels = (len < maxFillLen) ? len : maxFillLen,
                    bytes  = pixels * 2;
                descriptor[d].SRCADDR.reg       = (Uint32)pixelPtr + bytes;
                descriptor[d].BTCTRL.bit.SRCINC = 1;
...
//#endif // end !ESP8266
        ...
        do {
            ...
            yield(); // Periodic yield() on long fills
            while(pixelsThisPass--) {
                for(Uint16 bit=0, x=color; bit<16; bit++) {
        ...
        while(len--) {
 #if defined(__AVR__)
            for(Uint8 bit=0, x=hi; bit<8; bit++) {
            ...
            for(Uint8 bit=0, x=lo; bit<8; bit++) {
                if(x & 0x80) SPI_MOSI_HIGH();
                else         SPI_MOSI_LOW();
                SPI_SCK_HIGH();
                SPI_SCK_LOW();
        ...
        if(hi == lo) {
            ...
            *tft8.writePort = hi;
            ...
            if(!tft8.wide) {
                ...
                *(volatile Uint16 *)tft8.writePort = color;
            }
            while(len--) {
                TFT_WR_STROBE();
            }
...
*/
void Adafruit_SPITFT::writeFillRect(Int16 x, Int16 y,
...
*/
void Adafruit_SPITFT::writeFastHLine(Int16 x, Int16 y, Int16 w,
  Uint16 color) {
...
/*!
    ...
    @param  color  16-bit line color in '565' RGB format.
*/
void Adafruit_SPITFT::writeFastVLine(Int16 x, Int16 y, Int16 h,
  Uint16 color) {
...
*/
inline void Adafruit_SPITFT::writeFillRectPreclipped(Int16 x, Int16 y,
  Int16 w, Int16 h, Uint16 color) {
    setAddrWindow(x, y, w, h);
    writeColor(color, (Uint32)w * h);
}
...
/*!
    ...
    @param  color  16-bit pixel color in '565' RGB format.
*/
void Adafruit_SPITFT::drawPixel(Int16 x, Int16 y, Uint16 color) {
    // Clip first...
    if((x >= 0) && (x < _width) && (y >= 0) && (y < _height)) {
...
/*!
    ...
    @note   This repeats the writeFillRect() function almost in its entirety,
            ...
            that much code.
*/
void Adafruit_SPITFT::fillRect(Int16 x, Int16 y, Int16 w, Int16 h,
  Uint16 color) {
    if(w && h) {                            // Nonzero width and height?
        if(w < 0) {                         // If negative width...
            x +=  w + 1;                    //   Move X to left edge
...
*/
void Adafruit_SPITFT::drawFastHLine(Int16 x, Int16 y, Int16 w,
  Uint16 color) {
    if((y >= 0) && (y < _height) && w) { // Y on screen, nonzero width
        if(w < 0) {                      // If negative width...
            x +=  w + 1;                 //   Move X to left edge
...
/*!
    ...
    @note   This repeats the writeFastVLine() function almost in its
            ...
            transaction isn't performed at all if the line is rejected.
*/
void Adafruit_SPITFT::drawFastVLine(Int16 x, Int16 y, Int16 h,
  Uint16 color) {
    if((x >= 0) && (x < _width) && h) { // X on screen, nonzero height
        if(h < 0) {                     // If negative height...
            y +=  h + 1;                //   Move Y to top edge
...
/*!
    ...
    @param  color  16-bit pixel color in '565' RGB format.
*/
void Adafruit_SPITFT::pushColor(Uint16 color) {
    startWrite();
    SPI_WRITE16(color);
    endWrite();
}
...
*/
void Adafruit_SPITFT::drawRGBBitmap(Int16 x, Int16 y,
  Uint16 *pcolors, Int16 w, Int16 h) {

    Int16 x2, y2; // Lower-right coord
    if(( x             >= _width ) ||      // Off-edge right
       ( y             >= _height) ||      // " top
       ((x2 = (x+w-1)) <  0      ) ||      // " left
       ((y2 = (y+h-1)) <  0)     ) return; // " bottom

    Int16 bx1=0, by1=0, // Clipped top-left within bitmap
...
*/
Uint16 Adafruit_SPITFT::color565(Uint8 red, Uint8 green, Uint8 blue) {
 ...
 */
void Adafruit_SPITFT::sendCommand(Uint8 commandByte, Uint8 *dataBytes, Uint8 numDataBytes) {
    SPI_BEGIN_TRANSACTION();
    if(_cs >= 0) SPI_CS_LOW();
...
/*!
 ...
 @param   numDataBytes      The number of bytes we should send
 */
void Adafruit_SPITFT::sendCommand(Uint8 commandByte, const Uint8 *dataBytes, Uint8 numDataBytes) {
    SPI_BEGIN_TRANSACTION();
    if(_cs >= 0) SPI_CS_LOW();
 ...
 */
/**************************************************************************/
Uint8 Adafruit_SPITFT::readcommand8(Uint8 commandByte, Uint8 index) {
  Uint8 result;
  startWrite();
  SPI_DC_LOW();     // Command mode
...
*/
void Adafruit_SPITFT::spiWrite(Uint8 b) {
    if(connection == TFT_HARD_SPI) {
//#if defined(__AVR__)
...
//#endif
        ...
        *tft8.writePort = b;
        ...
        else           *(volatile Uint16 *)tft8.writePort = b;
#endif
        TFT_WR_STROBE();
    }
}
...
*/
void Adafruit_SPITFT::writeCommand(Uint8 cmd) {
    SPI_DC_LOW();
    spiWrite(cmd);
    SPI_DC_HIGH();
}
...
*/
Uint8 Adafruit_SPITFT::spiRead(void) {
    Uint8  b = 0;
    Uint16 w = 0;
    if(connection == TFT_HARD_SPI) {
//        return hwspi._spi->transfer((Uint8)0);
    } else if(connection == TFT_SOFT_SPI) {
        if(swspi._miso >= 0) {
            for(Uint8 i=0; i<8; i++) {
                SPI_SCK_HIGH();
  ...
  #if defined(HAS_PORT_SET_CLR)
                *(volatile Uint16 *)tft8.dirClr  = 0xFFFF; // Input state
                w = *(volatile Uint16 *)tft8.readPort;     // 16-bit read
                *(volatile Uint16 *)tft8.dirSet  = 0xFFFF; // Output state
  #else  // !HAS_PORT_SET_CLR
                *(volatile Uint16 *)tft8.portDir = 0x0000; // Input state
                w = *(volatile Uint16 *)tft8.readPort;     // 16-bit read
                *(volatile Uint16 *)tft8.portDir = 0xFFFF; // Output state
  #endif // end !HAS_PORT_SET_CLR
            }
...
inline void Adafruit_SPITFT::SPI_MOSI_HIGH(void) {
 ...
 #endif // end !HAS_PORT_SET_CLR
#else  // !USE_FAST_PINIO
    USBSTK5515_GPIO_setOutput(swspi._mosi, HIGH);
 #if defined(ESP32)
    for(volatile Uint8 i=0; i<1; i++);
 #endif // end ESP32
#endif // end !USE_FAST_PINIO
}
...
inline void Adafruit_SPITFT::SPI_SCK_HIGH(void) {
 ...
 #endif // end !HAS_PORT_SET_CLR
#else  // !USE_FAST_PINIO
    USBSTK5515_GPIO_setOutput(swspi._sck, HIGH);
 #if defined(ESP32)
    for(volatile Uint8 i=0; i<1; i++);
 #endif // end ESP32
#endif // end !USE_FAST_PINIO
...
inline Bool Adafruit_SPITFT::SPI_MISO_READ(void) {
 ...
 #endif // end !KINETISK
#else  // !USE_FAST_PINIO
    return USBSTK5515_GPIO_getInput(swspi._miso);
#endif // end !USE_FAST_PINIO
}
...
*/
void Adafruit_SPITFT::SPI_WRITE16(Uint16 w) {
    if(connection == TFT_HARD_SPI) {
//#if defined(__AVR__)
...
//#endif
        ...
        *tft8.writePort = w;
        ...
        if(!tft8.wide) {
            ...
            *(volatile Uint16 *)tft8.writePort = w;
        }
#endif
        TFT_WR_STROBE();
    }
...
*/
void Adafruit_SPITFT::SPI_WRITE32(Uint32 l) {
    if(connection == TFT_HARD_SPI) {
//#if defined(__AVR__)
...
//#endif
        ...
        *tft8.writePort = l;
        ...
        if(!tft8.wide) {
            ...
            *(volatile Uint16 *)tft8.writePort = l >> 16;
            TFT_WR_STROBE();
            *(volatile Uint16 *)tft8.writePort = l;
        }
#endif
        TFT_WR_STROBE();
    }
...
inline void Adafruit_SPITFT::TFT_WR_STROBE(void) {
 ...
 #endif // end !HAS_PORT_SET_CLR
#else  // !USE_FAST_PINIO
    USBSTK5515_GPIO_setOutput(tft8._wr, LOW);
    USBSTK5515_GPIO_setOutput(tft8._wr, HIGH);
#endif // end !USE_FAST_PINIO
}
...
inline void Adafruit_SPITFT::TFT_RD_LOW(void) {
 ...
 #endif // end !HAS_PORT_SET_CLR
#else  // !USE_FAST_PINIO
    USBSTK5515_GPIO_setOutput(tft8._rd, LOW);
#endif // end !USE_FAST_PINIO
}
