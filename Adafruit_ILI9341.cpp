...
*/
/**************************************************************************/
Adafruit_ILI9341::Adafruit_ILI9341(Int8 cs, Int8 dc, Int8 mosi,
        Int8 sclk, Int8 rst, Int8 miso) : Adafruit_SPITFT(ILI9341_TFTWIDTH, ILI9341_TFTHEIGHT, cs, dc, mosi, sclk, rst, miso) {
}
...
*/
/**************************************************************************/
Adafruit_ILI9341::Adafruit_ILI9341(Int8 cs, Int8 dc, Int8 rst) :
...
//  SPIClass *spiClass, Int8 dc, Int8 cs, Int8 rst) :
...
static const Uint8 PROGMEM initcmd[] = {
...
*/
/**************************************************************************/
void Adafruit_ILI9341::begin(Uint32 freq) {
    ...
    Uint8        cmd, x, numArgs;
    const Uint8 *addr = initcmd;
    while((cmd = pgm_read_byte(addr++)) > 0) {
        x = pgm_read_byte(addr++);
        numArgs = x & 0x7F;
        sendCommand(cmd, addr, numArgs);
        addr += numArgs;
        if(x & 0x80) USBSTK5515_wait(150);
    }
...
*/
/**************************************************************************/
void Adafruit_ILI9341::setRotation(Uint8 m) {
...
*/
/**************************************************************************/
void Adafruit_ILI9341::scrollTo(Uint16 y) {
    Uint8 data[2];
    data[0] = y >> 8;
    data[1] = y & 0xff;
    sendCommand(ILI9341_VSCRSADD, (Uint8*) data, 2);
}
 ...
 */
/**************************************************************************/
void Adafruit_ILI9341::setScrollMargins(Uint16 top, Uint16 bottom) {
  // TFA+VSA+BFA must equal 320
  if (top + bottom <= ILI9341_TFTHEIGHT) {
    Uint16 middle = ILI9341_TFTHEIGHT - top + bottom;
    Uint8 data[6];
    data[0] = top >> 8;
    data[1] = top & 0xff;
    data[2] = middle >> 8;
    ...
    data[4] = bottom >> 8;
    data[5] = bottom & 0xff;
    sendCommand(ILI9341_VSCRDEF, (Uint8*) data, 6);
  }
}
...
void Adafruit_ILI9341::setAddrWindow(
  Uint16 x1, Uint16 y1, Uint16 w, Uint16 h) {
    Uint16 x2 = (x1 + w - 1),
             y2 = (y1 + h - 1);
 ...
 */
/**************************************************************************/
Uint8 Adafruit_ILI9341::readcommand8(Uint8 commandByte, Uint8 index) {
  Uint8 data = 0x10 + index;
